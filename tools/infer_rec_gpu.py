...
def main(cfg):
    ...
    for file in get_image_file_list(global_config['infer_img']):
        ...
        if cfg['Architecture']['algorithm'] == "SRN":
            ...
            others = [
                torch.from_numpy(encoder_word_pos_list).to(device),
                torch.from_numpy(gsrm_word_pos_list).to(device),
                torch.from_numpy(gsrm_slf_attn_bias1_list).to(device),
                torch.from_numpy(gsrm_slf_attn_bias2_list).to(device)
            ]
        elif cfg['Architecture']['algorithm'] == "SAR":
            valid_ratio = np.expand_dims(batch[-1], axis=0)
            others = [torch.from_numpy(valid_ratio).to(device)]
        elif cfg['Architecture']['algorithm'] == "RobustScanner":
            valid_ratio = np.expand_dims(batch[1], axis=0)
            word_positons = np.expand_dims(batch[2], axis=0)
            others = [
                torch.from_numpy(valid_ratio).to(device),
                torch.from_numpy(word_positons).to(device),
            ]
...
if __name__ == '__main__':
    ...
    # -o
    # Global.pretrained_model=model/ch_PP-OCRv4_rec_server_train/best_accuracy.pth